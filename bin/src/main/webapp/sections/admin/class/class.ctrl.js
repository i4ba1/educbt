...
angular
  .module('app.core')
  .controller('ClassController', function($scope, $filter, ngTableParams, $stateParams, $state, classService, storageService, deferredService, errorHandle, $timeout, $uibModal) {
    ...
    function findClass(id) {
      var promise = deferredService.getPromise(classService.findClass(id, token));
      promise.then(
        function(response) {
    ...
    $scope.saveOrUpdate = function() {
      ...
      if ($scope.isUpdate) {
        promise = deferredService.getPromise(classService.updateClass(params));
        promise.then(
          function(response) {
        ...
        promise = deferredService.getPromise(classService.createClass(params));
        promise.then(
          function(response) {
            $state.go('admin.classMgmt');
          },
    ...
    $scope.delete = function() {
      var promise = deferredService.getPromise(classService.deleteClass($scope.selectedClass.id, token));
      promise.then(
        function(response) {
          $scope.showModal = false;
          findAll();
    ...
    $scope.importClass = function() {
      ...
      params.push(param);
      var promise = deferredService.getPromise(classService.importClass(params));
